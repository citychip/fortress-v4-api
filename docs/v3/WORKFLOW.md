...
**v3.1 · Updated 2026-07-15 (Trade Session Procedure gains the watchdog PAUSE switch (`touch /home/ubuntu/.fortress-watchdog-pause` in Phase 2, `rm` in Phase 3 — the auto-restart is now disarmed during trades, not just avoided) + the Cowork disconnect/execute/wait-for-reconnect protocol + the TradingView pre-flag.) Prior v3.0 · 2026-07-08 (Sprint 27 wiring is LIVE: matched-vertical roll/stop flags auto-exempt (`vertical_exempt` — no more manual ignoring) · briefing pacing is DYNAMIC (`vix_band`) · per-ticker β-DD block incl. `frozen[]` in the briefing · `weekly_close_above/below` alert types fire on the Friday bar only · scanner `days_to_earnings: null` renders "unverified", never "clear" (candidates route itself pending) · MCP v4.12.0. Prior v2.9: §v3.11 Cadence & Income Flow; v2.8: §Trade Session Procedure — the canonical analyze→exact-orders→manual-TWS-execution loop, incl. the gateway-vs-IBKR-Desktop single-session conflict.)**

> ## ⭐ v3.11 CADENCE & INCOME FLOW (canonical rules in `01_Portfolio_Strategy_v3_11.md` — the consolidated spec)
...
Run the full sweep while data is live. Do not log into IBKR Desktop until the order list is final. **In a Cowork session, flag up front — before starting the sweep — whether TradingView Desktop needs to be running** (any setup that needs chart levels: S/R, SMAs, trend). Tell the user to start TradingView and wait for their go-ahead before relying on `data_get_study_values`; don't assume it's open.
...
- **0. PAUSE THE WATCHDOG FIRST (2026-07-15):** `touch /home/ubuntu/.fortress-watchdog-pause` **before** logging into IBKR Desktop. While that flag exists `gateway_watchdog.sh` skips all probing/restarts, so it can never `docker restart cp-gateway` mid-trade and steal the session back from TWS. (Remove the flag in Phase 3.) This replaces the old "just don't restart manually" caution — the auto-restart is now disarmed for the session, not merely avoided.
- **Cowork protocol (2026-07-15):** in a Cowork/Claude session, the sequence is **Claude delivers the exact order list → Claude flags up front whether TradingView is needed → user disconnects the Fortress MCP → user executes in IBKR Desktop → Claude does NOT read/sync Fortress again until the user explicitly says "reconnected."** Claude must not reconnect or call any Fortress tool on its own between the order hand-off and the user's reconnect confirmation.
- The web_api **will** 401 when you log in — expected. With the watchdog paused it stays down cleanly until Phase 3.
...
0. **RESUME THE WATCHDOG:** log out of IBKR Desktop, then `rm -f /home/ubuntu/.fortress-watchdog-pause`. The watchdog resumes probing and (with the session now free) re-authenticates the gateway on its next cycle. In a Cowork session, the user removes the flag and reconnects the Fortress MCP, **then** tells Claude "reconnected" — only after that does Claude resume Fortress calls.
1. Restore the gateway if it hasn't self-healed → `docker restart cp-gateway` (or Parapet → Reconnect) → wait ~40s → `get_ibkr_status` must read `web_api` + authenticated (the resumed watchdog should also self-heal it; verify anyway).
