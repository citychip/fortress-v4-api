...
**Version 3.8.0 — June 1, 2026**

v3.8.0 introduces: explicit advisory-system framing (§1, §15.1); dual IV rank confirmation gate (§4); live GEX-by-strike as primary short-strike anchor (§5); vol skew directional gate (§5); delta entry/roll split codified — 0.25 entry target, 0.35 roll trigger (§5); MSFT formal de-risking plan (§7); standalone quantdata-mcp integration documented (§15.6); dashboard settings aligned with actual strategy (§15.6). Trading rules otherwise unchanged from v3.7.3.
...
Bid/ask spread ≤ 10% of mid on both legs. Open interest > 100 per leg. Underlying daily option volume: > 1K contracts (credit spreads), > 10K (LEAPS preferred).
...
3. Per-candidate deep dive (market hours): `qd_get_iv_rank(ticker)` [quantdata] to confirm IV rank; `qd_get_exposure_by_strike(ticker)` [quantdata] for live GEX walls; `qd_get_volatility_skew(ticker)` [quantdata] for skew direction.
...
### 15.6 Tool Stack Inventory (v4.2.0, June 2026)
...
| Server | Tools | Notes |
|---|---|---|
| `fortress-dashboard` | 62 tools (v4.2.0) | Portfolio, orders, journal, IBKR sync, market intel |
| `quantdata` | 26 widget tools | Live QuantData data via standalone quantdata-mcp |

**QuantData MCP — per-ticker status (tested 2026-06-01):**

| Tool | Status |
|---|---|
| `qd_get_iv_rank(ticker)` | ✓ confirmed per-ticker |
| `qd_get_volatility_skew(ticker)` | ⏱ pending market-hours test |
| `qd_get_exposure_by_strike(ticker)` | ⏱ pending market-hours test |
