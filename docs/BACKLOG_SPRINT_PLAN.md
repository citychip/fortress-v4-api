...
|---|---|---|---|---|
| 19.1 | 🔴 **β-weighted VEGA** (highest value, NOT built) — `get_portfolio_vega` β-weighted to SPY-IV; Briefing stat + SPY-hedge-sizing cross-check. You already compute portfolio vega + per-name betas. | `briefing.py`/`options_analytics.py` + MCP + Parapet | M | The risk dial a short-vega book lacks; `cfg("strategy.beta_vega_target")`. **Top next build.** |
