...
|---|---|---|---|---|
| 19.1 | **β-weighted VEGA** (highest value) — `get_portfolio_vega` β-weighted to SPY-IV; Briefing stat + SPY-hedge-sizing cross-check. You already compute portfolio vega + per-name betas. | `briefing.py`/`options_analytics.py` + MCP + Parapet | M | The one risk dial a short-vega book lacks; `cfg("strategy.beta_vega_target")` |
| 19.2 | **Correlated-cluster concentration** — sum `cfg("strategy.mag7_cluster")` exposure; warn > `cluster_concentration_warn_pct`. Briefing chip. | `briefing.py` + Parapet | S | Surfaces effective concentration the per-name caps miss |
| 19.3 | **Wire VRP gate to config** — `classify_signal` (scanner) + a pretrade VRP advisory read `cfg("strategy.vrp_*")` instead of hardcoded 5/0. | `iv_source.py`/scanner + `manage.py` | S | Thresholds already codified; this makes them live + tunable |
| 19.4 | **PMCC guardrails** — pretrade check: short strike > long breakeven (`pmcc_short_above_breakeven`); LEAP-roll alert at `leap_roll_delta` 0.70 / `leap_roll_dte` 120. | `manage.py` + alerts | M | §4 of the enhancements doc |
...
| 19.7 | **(Decision, not a build) Adopt 50% defined-risk profit target** — backtest `profit_target_pct` 80→50 via `journal_analytics.py`, then flip if confirmed. | settings + journal | S | `profit_target_pct_recommended = 50` recorded |
