...
| # | Item | File(s) | Effort | Dep |
|---|---|---|---|---|
| 22.1 | ✅ **DONE + LIVE-VERIFIED 2026-07-03.** `options_analytics.weekly_trend_state` (yfinance weekly-200, 6h cache, soft-fail) + `daily_trend_state` (daily 50/200-SMA, up/down/mixed trend + nearest key_level, 1h cache) → `GET /api/technical/gate` (`get_technical_gate`) returns per-name weekly Thesis-Stop state (hold/watch/act/unknown) + a `daily{}` block; MCP tool `get_technical_gate` live (relaunched). Unblocked 21.4 + the 21.1b trend nudge. **SKILL wired:** added the Technical-Gate step to `daily-post-open-briefing` (also dropped the OST line per user directive + refreshed the now-stale β-vega note). Live gate: all 7 book names above wk-200, MSFT `watch` (+2%). | `options_analytics.py` + MCP + `daily-post-open-briefing` task | M | ✅ |
| 22.2 | ✅ **DONE 2026-07-03.** `Fortress_MultiTimeframe_Procedure_v1_DRAFT.md` → **v1.1 (Active)**: launch-with-debug, dynamic watchlist, TN-signal reads, and the availability rule were already drafted; reconciled with shipped reality — the Technical Gate is now the one-call `get_technical_gate` tool (§6 step 3, §7.5, §9), and Monthly/4h are **no longer TV-only** (backend `get_chart_data` serves 1mo/4h per 22.5). Only the TN *signal flags* remain TV-only. | Procedure doc + Claude-side | S | ✅ |
| 22.3 | ✅ **CODE-COMPLETE 2026-07-03 (backend deploy + Parapet build).** The core banner already shipped (Sprint 0/19 `SourceBadge.tsx` — Live/Delayed/No-data from `/api/data-integrity`, source + fix pill). Added the missing bit: **positions last-sync age** — `/api/data-integrity` now returns `synced_at`/`sync_age_hours` (newest `_ibkr_sync_time` across the book), and `SourceBadge` shows a `⏱ Nh old` amber pill when the book is >2h stale **even if the live probe reads healthy** (the 18.8h-stale trap the integrity probe alone can't catch). `IntegrityData` type extended. **Not done:** "TradingView attached y/n" — not server-knowable (TV is desktop/Claude-side), out of scope for a server-driven badge. Write-gating on fallback deferred (§Open decision: warn, don't gate). | `options_analytics.py` + `fortress-parapet` | M | — |
...
| 22.5 | ✅ **CODE-COMPLETE 2026-07-03 (needs deploy).** Pulled the out-of-mount `app/routes/chart.py` into OneDrive as `chart_route.py` (Sprint-0 pattern; wired into `deploy_data_sources.sh` ROUTE_FILES + `sync_check.sh` MAP). `_fetch_ohlcv` now supports **`1mo`** (native yfinance passthrough) and **`4h`** (fetched as `1h` then resampled to session-aligned 4h OHLC bars); intraday lookback is clamped to yfinance's ~730d cap (1y+ → 180d); unknown intervals fall back to `1d`. Route Query docs + the `get_chart_data` MCP docstring updated (no MCP relaunch needed — the tool already proxies period/interval; relaunch only refreshes the docstring). Offline test `tests/test_sprint22_chart_interval.py` (exit 0, AST-extracts the real `_fetch_ohlcv` with a stubbed yfinance). | chart route + MCP docstring | M | — |
