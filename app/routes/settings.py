...
# ---------------------------------------------------------------------------
# Config JSON backup/restore  (Sprint v8.4 — K-02)
# ---------------------------------------------------------------------------
...
def config_backup():
    """
    Return the full current config as a timestamped JSON snapshot and
    write a copy to quant/config_backups/config_{timestamp}.json.
    ...
    Response shape:
        {
          ...
          "version": "v4",
          "config": { ...all sections... },
          "backup_file": "/path/to/config_backups/config_20260526T143000Z.json"
        }
    """
    from datetime import datetime as _dt, timezone as _tz
    backup_file = _auto_backup()
    return {
...
def config_restore(body: ConfigRestoreRequest):
    """
    Restore config from a JSON backup payload.

    Validates that the payload contains a non-empty `config` dict with at
    least one known section, then resets to factory defaults and applies
    the backup values section by section.
    ...
    unknown = set(body.config.keys()) - KNOWN_SECTIONS
    if unknown:
        raise HTTPException(
            status_code=400,
            detail=f"Unknown config sections in payload: {sorted(unknown)}. "
                   f"Accepted: {sorted(KNOWN_SECTIONS)}",
        )

    bad_sections = [s for s, v in body.config.items() if not isinstance(v, dict)]
    if bad_sections:
        raise HTTPException(
            status_code=400,
            detail=f"Config sections must be objects (dicts). Bad sections: {bad_sections}",
        )

    # Take a safety backup of the current config before wiping it
    _auto_backup()
...
# ---------------------------------------------------------------------------
# Config auto-backup helper  (Sprint v8.4 — K-02)
# ---------------------------------------------------------------------------
...
def _auto_backup() -> str | None:
    """
    Write a timestamped config snapshot to quant/config_backups/.
    Silently swallows all errors — auto-backup must never block a write.
    Prunes to the last 10 backup files.
    Returns the file path written, or None on failure.
    """
    try:
        from app.services import state as _state_b
        backup_dir = _state_b.BASE_DIR / "config_backups"
        backup_dir.mkdir(parents=True, exist_ok=True)

        ts  = _dt_b.now(_tz_b.utc).strftime("%Y%m%dT%H%M%SZ")
        payload = {
            "timestamp": _dt_b.now(_tz_b.utc).isoformat(),
            "version":   "v4",
            "config":    config_store.get_all(),
        }
        path = backup_dir / f"config_{ts}.json"
        path.write_text(_json_b.dumps(payload, indent=2))

        # Prune: keep only the last 10 backups
